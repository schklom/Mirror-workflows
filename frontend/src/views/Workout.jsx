...
function ActiveWorkout() {
  ...
  const moveCurrentUnit = direction => {
    ...
    // Invalidate an old timed callback before indexes shift, and cancel a displaced rest timer.
    ui.stopWork()
    ui.stopRest()
    update(s => {
      const moved = moveActiveWorkoutUnit(s.active, s.active?.cur, direction)
      if (moved) progressHighWater.current = moved.indices.map(index => progressHighWater.current[index])
    }, true)
  }
