...
website/   static marketing site (plain HTML/CSS/JS), deployed separately (GitHub Pages).
...
CI gate is `.gitea/workflows/test.yml` (Gitea Actions — the copy in `.github/workflows/test.yml`
is kept dormant for GitHub). Both just run `npm ci && npm test` in `frontend/` on Node 22, the
same version as `web/Dockerfile` / `api/Dockerfile` (`node:22-alpine`). `.gitlab-ci.yml` on
GitLab (canonical remote, see README) additionally builds/publishes Docker images, packages the
